...
def estimate_component(comp):
    ...
    sns.kdeplot(classifier.predict_proba(envs)[y==1,1], ax=axes[5])
    sns.kdeplot(classifier.predict_proba(envs)[y == 0, 1], ax=axes[5])
    axes[5].set_xlim(0,1)
    plt.tight_layout()
    plt.show()
...
class TwoSatesClassifier(LSLTransformer):
    def __init__(self, *args, **kwargs):
        ...
        inlet_channels = [ch.upper().split('-')[0] for ch in self.inlet.get_channels_labels()]
...
transformer = TwoSatesClassifier('Mitsar', 'BCIState', ['BCI'])
while True:
    transformer.update()
    sleep(0.05)